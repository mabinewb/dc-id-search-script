...
// @version      2.6
// @description  닉네임 클릭 메뉴와 검색창에 식별코드(UID) 및 IP 검색 옵션을 추가하고, 30페이지씩 연속 스캔합니다.
// @author       uid1000
...
(function() {
    ...
    function init() {
        ...
        if (queryFromUrl && window.location.pathname.includes('/lists')) {
            setTimeout(() => executeInstantSearch(queryFromUrl), 500);
        }
        injectSearchType();
        injectUserMenu();
    }
    ...
    function handleSearchRedirection(val) {
        if (window.location.pathname.includes('/lists')) {
            ...
            window.location.href = `${listUrl}?id=${gallId}&uid_search=${val}`;
        }
    }
    ...
    async function startDeepSearch(query, isNewSearch) {
        ...
        if (!listTable) return;
        if (isNewSearch) {
            listTable.innerHTML = `<tr><td colspan="5" style="padding:50px; text-align:center;">[${query}] 스캔 중... (1~${SCAN_STEP}페이지)</td></tr>`;
            removeNextButton();
        }
        const gallId = new URLSearchParams(window.location.search).get('id');
        ...
        try {
            ...
            if (foundRows.length === 0 && isNewSearch) {
                listTable.innerHTML = `<tr><td colspan="5" style="padding:50px; text-align:center;">해당 범위 내에 검색 결과가 없습니다.</td></tr>`;
            }
